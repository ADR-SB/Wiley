...
SELECT ABS(O.shares*O.price) AS 'absolut order',
O.status,
O.symbol,
O.orderTime AS 'trade data',
U.Uname
FROM `Order` O
Join User U ON U.userid=O.userid
WHERE O.status NOT IN ('canceled', 'partially canceled')
ORDER BY 'absolut order' DESC;
/*
...
SELECT O.orderid,
       ...
       SUM(F.share) AS 'outstanding shares left'
FROM `Order` O
...
SELECT o.symbol,
       u.uname AS username,
       r.name AS role,
       SUM(f.share) AS 'number of filled shares'
FROM `Order` o
JOIN User u ON o.userid = u.userid
JOIN UserRoles ur ON u.userid = ur.userid
JOIN Role r ON ur.roleid = r.roleid
